 ...
 * @param title 
 */
export async function bench(func: () => any, title?: string): Promise<void> {
    const start = performance.now();
    await func();
    const end = performance.now();

    console.log(`${title ? `[${title}] `:""}計測結果: ${(Math.round(end-start)/1000).toLocaleString()} 秒 (${Math.round(end - start).toLocaleString()} ms)`);
}